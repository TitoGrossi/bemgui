    ...
    for percentage_point in percentages:
        if count == 0:
            if last_point_from_previous_edge:
                ...
                first_point = bemgui.dcel.meshgenerator.GraphicalElements.meshExtremityPoint(half_edge.pointAtPercent(percentage_point), 0)
            count += 1
        elif count == 1:
            middle_point = bemgui.dcel.meshgenerator.GraphicalElements.meshMiddlePoint(half_edge.pointAtPercent(percentage_point))
            count += 1
        elif count == 2:
            final_point = bemgui.dcel.meshgenerator.GraphicalElements.meshExtremityPoint(half_edge.pointAtPercent(percentage_point), 0)
            element = mesh_element = bemgui.dcel.meshgenerator.GraphicalElements.meshElement(first_point, middle_point, final_point)
